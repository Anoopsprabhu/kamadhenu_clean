...
export default function Footer() {
  ...
  return (
    <footer
      ...
      style={{
        background: "linear-gradient(175deg, #C85212 0%, #8A2B06 100%)",
      }}
    >
      {/* Decorative top */}
      ...
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-8">
        <div className="grid md:grid-cols-4 gap-10 mb-10">
            ...
            <ul className="space-y-2">
              {quickLinks.map((l) => (
                <li key={l.label}>
                  <Link
                    href={l.href}
                    className="font-lora text-white/80 hover:text-saffron-200 text-sm transition-colors"
                  >
                    {l.label}
                  </Link>
                </li>
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-start gap-2">
                <MapPin
                  size={14}
                  className="text-saffron-200 mt-0.5 flex-shrink-0"
                />
          ...
          <div className="flex flex-col gap-4">
            <h4 className="font-cinzel text-white text-sm font-semibold tracking-wide">
              {t("Support the Mission", "ಮಿಷನ್ ಬೆಂಬಲಿಸಿ")}
            </h4>
            <Link
              href="/#donate"
        ...
        <div className="border-t border-white/20 pt-6 flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <p className="font-lora text-white/70 text-xs text-center">
            {t("Supported by", "ಬೆಂಬಲಿಸಿದವರು")}{" "}
            <span className="text-saffron-200">
              Lcode Technologies Pvt. Ltd., Mangalore
            </span>
          </p>
        </div>
      </div>
