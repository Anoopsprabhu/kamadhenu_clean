...
const bankDetails = {
  ...
  branch:  'Brahmavar Branch, Udupi',
  account: 'XXXX XXXX XXXX (contact us)',
  ifsc:    'SBIN0XXXXXX (contact us)',
  upi:     'kamadhenutrust@sbi (placeholder)',
}
...
export default function Donate() {
  ...
  return (
    <section id="donate" className="py-16 px-6" style={{ background: '#FFFFFF' }}>
      <div className="max-w-6xl mx-auto">
        <ScrollReveal>

        {/* Header */}
        <div className="text-center mb-12">
          <p className="font-cinzel text-saffron-500 text-xs tracking-[0.3em] uppercase mb-3">{t('Join the Seva', 'ಸೇವೆಯಲ್ಲಿ ಸೇರಿ')}</p>
        ...
        <div className="flex flex-col lg:flex-row gap-10 max-w-5xl mx-auto items-stretch justify-center">
              ...
              <table className="w-full font-lora text-base">
                <tbody>
                  {[
                    ...
                    <tr key={k} className="border-b border-saffron-100/50 last:border-0 relative hover:bg-saffron-50/50 transition-colors">
                      <td className="py-4 text-saffron-800 pr-4 whitespace-nowrap font-medium text-sm md:text-base">{k}</td>
                      <td className="py-4 text-saffron-900 text-sm md:text-base font-semibold text-right">{v}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-saffron-50 rounded-3xl p-10 border border-saffron-200/60 shadow-xl flex flex-col items-center justify-center text-center w-full lg:w-1/2">
            <h3 className="font-cinzel font-bold text-saffron-900 text-xl mb-6 pb-4 w-full border-b border-saffron-200/50">{t('UPI Payment', 'UPI ಪಾವತಿ')}</h3>
            {/* QR placeholder */}
            <div className="w-48 h-48 bg-white rounded-2xl flex items-center justify-center border-2 border-saffron-200/80 mb-6 shadow-sm overflow-hidden p-2">
              <div className="w-full h-full bg-white rounded-xl flex items-center justify-center border border-dashed border-saffron-300">
                <span className="font-cinzel font-semibold text-saffron-500 text-sm md:text-base text-center px-4">
                  {t('QR Code Placeholder', 'QR ಕೋಡ್ ಪ್ಲೇಸ್‌ಹೋಲ್ಡರ್')}
                </span>
              </div>
            </div>
            <p className="font-lora text-saffron-800 text-base mb-2 font-medium">{t('Or send directly to', 'ಅಥವಾ ನೇರವಾಗಿ ಕಳುಹಿಸಿ')}</p>
            <div className="bg-white px-6 py-3 rounded-full border border-saffron-200 shadow-sm">
               <p className="font-cinzel font-bold text-saffron-500 text-lg tracking-wide">{bankDetails.upi}</p>
            </div>
            ...
            <p className="font-lora text-saffron-800 text-xs mt-8 bg-white p-3 rounded-lg border border-saffron-100/50 italic leading-relaxed">
              {t('* Please send a confirmation email or message after donating so we can issue your receipt.', '* ದಯವಿಟ್ಟು ದಾನ ಮಾಡಿದ ನಂತರ ದೃಢೀಕರಣ ಇಮೇಲ್ ಅಥವಾ ಸಂದೇಶವನ್ನು ಕಳುಹಿಸಿ ಇದರಿಂದ ನಾವು ನಿಮ್ಮ ರಸೀದಿಯನ್ನು ನೀಡಬಹುದು.')}
            </p>
          </div>
